...
const state = {
    ...
    navTree: [],            // 导航菜单树工作副本（专用树形编辑器）
    navCollapsed: new Set(),// 折叠节点的路径集合
    navSelected: null,      // 当前选中节点的路径数组（如 [0,2]），null=未选中
    navDirty: false,        // 导航树是否有未保存修改
};
...
function renderModuleCards(m, data) {
    switch (m.id) {
        case "posts":
            return renderPostCards(data);
        case "gallery":
            return renderGalleryCards(data);
        case "books":
            return renderBookCards(data);
        case "games":
            return renderGameCards(data);
        case "movies":
            return renderMovieCards(data);
        case "friends":
            return renderFriendCards(data);
        case "plans":
            return renderPlanCards(data);
        case "travel":
            return renderTravelCards(data);
        case "website":
            return renderWebsiteCards(data);
        case "changelog":
            return renderChangelogCards(data);
        case "about":
            return renderAboutCard(data);
        default:
            return renderFallbackList(data);
    }
}
...
function bindCardGrid(panel) {
    ...
    panel.querySelectorAll("[data-edit]").forEach((el) => (el.onclick = (e) => {
        e.stopPropagation();
        openEdit(el.dataset.edit);
    }));
    panel.querySelectorAll("[data-del]").forEach((el) => (el.onclick = (e) => {
        e.stopPropagation();
        ...
        $$("#modal-body [data-restore]").forEach((b) =>
            (b.onclick = () => {
                confirmModal("恢复确认", `确定用备份「${b.dataset.restore}」覆盖恢复吗？`, async () => {
                    try {
                        await api(`/api/content/${state.module.id}/restore`, {
                            method: "POST",
                            body: {backupName: b.dataset.restore}
                        });
                        toast("恢复成功", "ok");
                        closeModal();
                        await renderModuleList();
...
const CFG_WORD_ZH = {
    ...
    pinned: "置顶", categoryId: "分类 ID", featured: "推荐", order: "排序值", circle: "推文",
    moments: "朋友圈", bills: "账单",
};
...
function emitAccessChange(keyPath) {
    ...
    if (!root) return;
    const arr = [];
    root.querySelectorAll("[data-acc-p]").forEach((el) => {
        if (el.checked) arr.push(el.dataset.accP);
    });
...
const CFG_WIDGET_EXTRA_FIELDS = [
    ...
    {
        key: "hideOnNonPostPage",
        label: "非文章页隐藏（hideOnNonPostPage）",
        hint: "true / false（true=仅文章页显示）",
        def: true
    },
];
...
/** 站点功能开关（siteConfig.pages）可选项，供节点「关联功能开关」下拉 */
const NAV_PAGE_KEYS = [
    ["", "（不关联，始终按 enabled 显示）"],
    ["friends", "友链页面开关"],
    ["moments", "朋友圈页面开关"],
...
/** 沿「路径段」数组在树中取节点（段为数字=数组下标，字符串=对象键） */
function navGet(tree, segs) {
    let o = tree;
    for (const s of segs) {
        if (o == null) return null;
        if (typeof s === "number") o = o[s];
        else if (typeof s === "string") o = o ? o[s] : undefined;
        else o = o[s];
    }
    return o;
}

/** 把工作副本中的节点整理为可写回 TS 的干净结构 */
function navCleanNode(n) {
    const out = {name: stringish(n.name), url: stringish(n.url)};
    if (n.enabled === true || n.enabled === false) out.enabled = !!n.enabled;
    else out.enabled = true;
    // 外链自动判断：URL 以 http(s):// 或 // 开头即视为外链（博客端 url() 原样返回，新窗口打开）
    if (/^(https?:)?\/\//.test(out.url)) out.external = true;
    if (n.icon) out.icon = String(n.icon);
    if (n.pageKey) out.pageKey = String(n.pageKey);
    if (Array.isArray(n.children) && n.children.length) {
        out.children = n.children.map(navCleanNode);
    }
    return out;
}
...
/** 生成左侧菜单树列表 HTML：每行=缩进+折叠箭头+名称（+外链/隐藏角标），点击选中 */
function navListHtml(list, segs, collapsed, selectedKey) {
    if (!Array.isArray(list) || list.length === 0) {
        return `<div class="array-hint">${segs.length === 0 ? "（暂无菜单，点下方＋添加）" : "（无子项）"}</div>`;
    }
    return list
        .map((node, i) => {
            // segs 指向真实路径：根列表路径为 []，子列表路径为 [父..., "children"]
            const nodeSegs = [...segs, i];
            const nodeKey = nodeSegs.join("|");
            const dataSegs = JSON.stringify(nodeSegs);
            const kids = Array.isArray(node.children) ? node.children : [];
            const hasKids = kids.length > 0;
            const open = !collapsed.has(nodeKey);
            const selected = selectedKey === nodeKey;
            const hidden = node.enabled === false;
            const isExt = /^(https?:)?\/\//.test(stringish(node.url));
            let row = `<div class="nt-row${selected ? " nt-sel" : ""}" data-nsf="select" data-segs="${esc(dataSegs)}" style="--depth:${segs.length}">
        ${hasKids ? `<button type="button" class="nt-arrow" data-nsf="toggle-kids" data-segs="${esc(dataSegs)}" title="展开/收起">${open ? "▾" : "▸"}</button>` : `<span class="nt-arrow nt-arrow-empty"></span>`}
        <span class="nt-name">${esc(stringish(node.name) || "（未命名）")}</span>
        ${isExt ? `<span class="nt-tag">外链</span>` : ""}
        ${hidden ? `<span class="nt-tag nt-tag-hide">隐藏</span>` : ""}
      </div>`;
            if (hasKids && open) row += navListHtml(kids, nodeSegs.concat(["children"]), collapsed, selectedKey);
            return row;
        })
        .join("");
}

/** 右侧编辑面板：选中节点的字段表单 + 操作按钮 */
function navEditPanelHtml(node, segs) {
    if (!node || !segs) {
        return `<div class="array-hint">← 请先在左侧点选一个菜单项进行编辑</div>`;
    }
    const dataSegs = JSON.stringify(segs);
    return `<div class="form-row"><div class="row-head"><label>菜单名称</label></div>
        <input type="text" data-nsf="name" data-segs="${esc(dataSegs)}" value="${esc(stringish(node.name))}" placeholder="菜单名"></div>
      <div class="form-row"><div class="row-head"><label>链接</label><span class="kpath">站内路径如 /posts/，或 http(s):// 外链</span></div>
        <input type="text" data-nsf="url" data-segs="${esc(dataSegs)}" value="${esc(stringish(node.url))}" placeholder="/路径/ 或 https://"></div>
      <div class="form-row"><div class="row-head"><label>图标（可选）</label><span class="kpath">iconify 图标名，如 material-symbols:home</span></div>
        <input type="text" data-nsf="icon" data-segs="${esc(dataSegs)}" value="${esc(stringish(node.icon))}" placeholder="material-symbols:home"></div>
      <div class="form-row"><div class="row-head"><label>关联页面开关</label><span class="kpath">对应功能页关闭时该菜单隐藏（如 circle/friends）</span></div>
        <select class="nn-pagekey" data-nsf="pageKey" data-segs="${esc(dataSegs)}">${
            NAV_PAGE_KEYS.map(([k, lbl]) => `<option value="${esc(k)}" ${node.pageKey === k ? "selected" : ""}>${esc(lbl)}</option>`).join("")
        }</select></div>
      <label class="nc-toggle"><input type="checkbox" data-nsf="enabled" data-segs="${esc(dataSegs)}" ${node.enabled === false ? "" : "checked"}> 在导航中显示</label>
      <div class="nt-ops">
        <button type="button" class="btn btn-sm btn-info" data-nsf="addchild" data-segs="${esc(dataSegs)}">＋ 添加子项</button>
        <button type="button" class="btn btn-sm" data-nsf="addsib" data-segs="${esc(dataSegs)}">＋ 添加同级</button>
        <button type="button" class="btn btn-sm" data-nact="up" data-segs="${esc(dataSegs)}">↑ 上移</button>
        <button type="button" class="btn btn-sm" data-nact="down" data-segs="${esc(dataSegs)}">↓ 下移</button>
        <button type="button" class="btn btn-sm btn-danger" data-nact="del" data-segs="${esc(dataSegs)}">✕ 删除</button>
      </div>`;
}

/** 渲染「导航菜单」专用树形编辑器：左=菜单树列表，右=选中项编辑面板 */
function renderNavMenuPanel(read) {
    const panel = $("#config-panel");
    let html = "";
    if (read.saveNote) html += `<div class="save-note">📌 ${esc(read.saveNote)}</div>`;
    const menusRaw = read.fields.find((f) => f.keyPath === "menus");
    state.navTree = (menusRaw && Array.isArray(menusRaw.value) ? menusRaw.value : []);
    state.navCollapsed = new Set();
    state.navSelected = null;
    html += `<div class="form-row"><div class="row-head"><label>顶部导航菜单树</label><span class="kpath">menus（左侧点选菜单 → 右侧编辑其名称/链接/图标/显隐/关联页面；↑↓排序、＋增项、✕删除）</span></div>
    <div class="nav-split">
      <div class="nav-tree" data-nav-root>${navListHtml(state.navTree, [], state.navCollapsed, null)}</div>
      <div class="nav-edit" data-nav-edit>${navEditPanelHtml(null, null)}</div>
    </div>
    <button type="button" class="btn btn-sm btn-info nav-root-add" data-nsf="addroot">＋ 添加顶级菜单</button>
  </div>`;
    html += `<div class="config-savebar">
    <button class="btn btn-primary" id="btn-nav-save">💾 保存导航菜单</button>
    <span class="sb-count" id="nav-changed-count">${state.navDirty ? "已修改（未保存）" : ""}</span>
  </div>`;
    panel.innerHTML = html;
    bindNavMenuEditor(panel);
}

/** 提交导航树（整体写回 menus） */
async function persistNavTree(msg) {
    try {
        const value = state.navTree.map(navCleanNode);
        const data = await api(`/api/config/${state.target.targetId}`, {
            method: "POST",
            body: {changes: [{keyPath: "menus", value}]},
        });
        toast(`${msg}，已保存（${data.saved} 项）`, "ok");
...
function setNavDirty() {
    state.navDirty = true;
    const el = $("#nav-changed-count");
    if (el) el.textContent = "已修改（未保存）";
}

function bindNavMenuEditor(panel) {
    // 左侧行：点击选中
    panel.querySelectorAll("[data-nsf=select]").forEach((row) => {
        row.addEventListener("click", () => {
            state.navSelected = JSON.parse(row.dataset.segs);
            rerenderNavTree(panel);
        });
    });
    // 折叠/展开子项（箭头）
    panel.querySelectorAll("[data-nsf=toggle-kids]").forEach((b) => {
        b.addEventListener("click", (e) => {
            e.stopPropagation();
            const key = JSON.parse(b.dataset.segs).join("|");
            if (state.navCollapsed.has(key)) state.navCollapsed.delete(key);
            else state.navCollapsed.add(key);
            rerenderNavTree(panel);
        });
    });
    // 右侧编辑表单：就地更新工作副本，不重渲染（避免失焦）
    panel.querySelectorAll("[data-nav-edit] [data-nsf]").forEach((el) => {
        const f = el.dataset.nsf;
        if (f === "addchild" || f === "addsib") return;
        const apply = () => {
            const node = navGet(state.navTree, JSON.parse(el.dataset.segs));
            if (!node) return;
            if (f === "name") node.name = el.value;
            else if (f === "url") node.url = el.value;
            else if (f === "enabled") node.enabled = el.checked;
            else if (f === "icon") node.icon = el.value === "" ? undefined : el.value;
            else if (f === "pageKey") node.pageKey = el.value === "" ? undefined : el.value;
            setNavDirty();
        };
    ...
    // 结构操作：添加顶级菜单
    panel.querySelectorAll("[data-nsf=addroot]").forEach((b) => {
        b.addEventListener("click", () => {
            state.navTree.push({name: "", url: "", enabled: true});
            state.navSelected = [state.navTree.length - 1];
            setNavDirty();
            rerenderNavTree(panel);
        });
    });
    // 添加子项 / 添加同级（作用于当前选中节点）
    panel.querySelectorAll("[data-nsf=addchild]").forEach((b) => {
        b.addEventListener("click", () => {
            const segs = JSON.parse(b.dataset.segs);
            const node = navGet(state.navTree, segs);
            if (!node) return;
            if (!Array.isArray(node.children)) node.children = [];
            node.children.push({name: "", url: "", enabled: true});
            state.navCollapsed.delete(segs.join("|"));
            state.navSelected = [...segs, node.children.length - 1];
            setNavDirty();
            rerenderNavTree(panel);
        });
    });
    panel.querySelectorAll("[data-nsf=addsib]").forEach((b) => {
        b.addEventListener("click", () => {
            const segs = JSON.parse(b.dataset.segs);
            if (segs.length === 0) return;
            const idx = segs[segs.length - 1];
            const parent = segs.length > 1 ? navGet(state.navTree, segs.slice(0, -1)) : state.navTree;
            if (!Array.isArray(parent)) return;
            parent.splice(idx + 1, 0, {name: "", url: "", enabled: true});
            state.navSelected = [...segs.slice(0, -1), idx + 1];
            setNavDirty();
            rerenderNavTree(panel);
        });
    });
    // 上移/下移/删除（作用于当前选中节点）
    panel.querySelectorAll("[data-nact]").forEach((b) => {
        b.addEventListener("click", () => {
            const segs = JSON.parse(b.dataset.segs);
            ...
            const idx = segs[segs.length - 1];
            const parent = segs.length > 1 ? navGet(state.navTree, segs.slice(0, -1)) : state.navTree;
            if (!Array.isArray(parent) || idx == null) return;
            if (act === "del") {
                parent.splice(idx, 1);
                state.navSelected = null;
            } else if (act === "up") {
    ...
    // 保存
    const saveBtn = panel.querySelector("#btn-nav-save");
    if (saveBtn) saveBtn.onclick = () => persistNavTree("导航菜单已修改");
}

function rerenderNavTree(panel) {
    const selKey = state.navSelected ? state.navSelected.join("|") : null;
    const treeRoot = document.querySelector("[data-nav-root]");
    if (treeRoot) treeRoot.innerHTML = navListHtml(state.navTree, [], state.navCollapsed, selKey);
    const editBox = document.querySelector("[data-nav-edit]");
    if (editBox) editBox.innerHTML = navEditPanelHtml(state.navSelected ? navGet(state.navTree, state.navSelected) : null, state.navSelected);
    bindNavMenuEditor(panel);
}

function renderConfigPanel(read) {
    if (read.targetId === "navMenu") {
        renderNavMenuPanel(read);
        return;
    }
...
function init() {
    ...
    // Tab 切换
    $$(".appbar-tabs a[data-tab]").forEach((a) => (a.onclick = (e) => {
        e.preventDefault();
